			...
			foreach ($this->options['layers'] as $i=>$l)
			{
				...
				{					
					...
					{																	
						...
						{
							$this->options['layers'][$i]['urls'] = ["http://".($module->proxyhost?$module->proxyhost:$module->ipaddress.":".$module->ports[0])."/iyo".$layer->id."_".preg_replace('/[^a-zA-Z0-9]/','_',strtolower($layer->title))."/{z}/{x}/{y}.".(in_array($layer->data->type,[0,3])?'json':'png')];	
						}
						else
						{
						...
						{							
							if ($module->tileServer == 'nodejs')
							{
								$this->options['layers'][$i]['urls'] = ["http://".($module->proxyhost?$module->proxyhost:$module->ipaddress.":".$module->ports[0])."/iyo".$layer->id."_".preg_replace('/[^a-zA-Z0-9]/','_',strtolower($layer->title))."/{z}/{x}/{y}.".(in_array($lconfigs[0]['type'],['geojson'])?'json':'png')];
							}
							else
							{
								$baseUrl = \Yii::getAlias($module->baseUrl);
